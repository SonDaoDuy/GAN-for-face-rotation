...
class GANormalization(BaseModel):
	...
	def _forward_G(self, keep_data_for_visuals):
		...
		#cycle
		rec_real_img, _ = self._G.forward(fake_img, self._real_heat_map)
	...
	def _forward_D(self):
		...
		#print(fake_img.size())
		#D(real_I)
		d_fake_real_img = self._D.forward(self._real_img, self._real_heat_map)
		# print(d_fake_real_img.size())
		# print(self._real_img.size())
		self._loss_d_rec_real = self._criterion_cycle(d_fake_real_img, self._real_img)
		#self._loss_d_rec_real = torch.mean(torch.abs(self._real_img - d_fake_real_img))
		#D(fake_I)
		# self._clone_of_fake_img.resize_(fake_img.size()).copy_(fake_img.data)
		# clone_image = Variable(self._clone_of_fake_img)
		d_fake_fake_img = self._D.forward(fake_img.detach(), self._desired_heat_map)
		self._loss_d_rec_fake = self._criterion_cycle(d_fake_fake_img, fake_img.detach()) # co can detech o day ko???
		...
		return self._loss_d_adv, fake_img, d_fake_fake_img, d_fake_real_img
